...
export const getCategoriesAndDocuments = async() =>{
  ...
  querySnapshot.docs.map((doc) =>{
    ...
    return fetchUtils.callApi('GET','/category',{id}).then((response)=>{
      if(response.ok){
        categories[title]=[...categories[title], ...response.json().results];
        return setDoc(docCategory,{items_count: response.json().items_count},{merge:true})
      }
    })
  })
  return Promise.allSettled(querySnapshot.docs.map( (doc)=> doc.ref.update({ items_count : ref(db, 'categories/' + doc
  return Promise.allSettled(querySnapshot.docs.map( (doc)=> doc.ref.update({ items_count : ref(docCategory,"items_count")
}
